...
- (void) stop
{
    ...
    _logFileNameLocations=[self logFileNameLocations];
     NSLog(@"Dispatch Location -Stopped, vamos a grabar sobre %@",_logFileNameLocations);
...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation
{
    ...
    //NSLog(@"LOC-DISPATCH:updating location para: %@",[self logFileNameLocations]);
    ...
    if (self.logLocationData && !self.isRunningDemo) {
        ...
        NSLog(@"LOC-DISPATCH: adding location to file");
    }
...
{    
    if ( cacheType == kLocationsCache) {
        ...
        NSLog(@"Archivo guardado en : %@",_logFileNameLocations);
...
//    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    @autoreleasepool {
        // create log file name
        _logFileNameLocations = Retain([[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:logFileName]);
        NSLog(@"Ruta Log file de locationDispatch: %@",[[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:logFileName]);
        _loggedLocations = [NSKeyedUnarchiver unarchiveObjectWithFile:_logFileNameLocations];
